...
% If sin of theta is 1 or -1; we have to calculate the angles in a different way.
if(sind(pitch) == 1 || sind(pitch) == -1) 
    % %Since roll and yaw are now cannot be distinguished (because sin(theta)
    %= +-1), we can assume the value of one angle.
    roll = 0;  
